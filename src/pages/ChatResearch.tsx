...
const ChatResearch = () => {
  ...
  return <div className="min-h-screen w-full bg-gradient-to-br from-[#0a0e17] to-[#131b2e] text-[#f5f7fa]">
      {/* Header */}
          ...
          <div className="flex items-center gap-2">
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className={`rounded-full ${!hasApiKey ? 'animate-pulse text-amber-400' : ''}`} aria-label="Settings">
      ...
      <div className="flex h-[calc(100vh-64px-70px)] overflow-hidden">
        ...
        <div className={`relative flex h-full flex-col transition-all duration-300 ${isPanelExpanded ? "w-0 overflow-hidden" : isVisualizationOpen ? "w-1/2" : "w-full"}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {messages.length === 0 ? <div className="flex h-full flex-col items-center justify-center gap-4 text-center text-gray-400">
                ...
                <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-2">
                  {["How will rising interest rates affect tech stocks?", "Compare Apple and Microsoft's performance this quarter", "Explain the impact of Fed announcements on banking stocks", "What regions are affected by semiconductor shortages?", "Show me historical patterns of market reactions to similar events"].map(question => <Button key={question} variant="outline" className="h-auto justify-start border-gray-700 py-3 text-left text-xs hover:bg-gray-800 md:text-sm" onClick={() => handleSuggestedQuestion(question)} disabled={!hasApiKey}>
                      <MessageCircle className="mr-2 h-3.5 w-3.5" />
                      {question}
                    </Button>)}
                </div>
                
                {!hasApiKey && <div className="mt-4 rounded-md border border-amber-900/50 bg-amber-900/20 p-3 text-sm text-amber-200">
              ...
              </div> : messages.map((message, index) => <ChatMessage key={index} message={message} />)}
          ...
          <div className="border-t border-white/10 bg-black/20 p-4 backdrop-blur-sm">
            <form onSubmit={handleSendMessage} className="space-y-4">
              <div className="relative">
                <Input ref={inputRef} value={inputMessage} onChange={e => setInputMessage(e.target.value)} placeholder="Ask about financial markets, stocks, or economic events..." className="bg-gray-900 pr-12 text-white" disabled={loading || !hasApiKey} />
                <Button type="submit" size="icon" className="absolute right-1 top-1 h-8 w-8 rounded-md" disabled={loading || !inputMessage.trim() || !hasApiKey}>
                  <Send className="h-4 w-4" />
                </Button>
              </div>

              {/* Suggested Questions */}
              {suggestedQuestions.length > 0 && <SuggestedQuestions questions={suggestedQuestions} onSelectQuestion={handleSuggestedQuestion} />}
            </form>
          </div>

          {/* Toggle Visualization Button (Mobile) */}
          <Button variant="secondary" size="icon" className="absolute right-4 top-4 block md:hidden" onClick={toggleVisualization}>
            {isVisualizationOpen ? <ChevronRight className="h-4 w-4" /> : <ChartLine className="h-4 w-4" />}
          </Button>
        </div>

        {/* Expand/Collapse Button */}
        {isVisualizationOpen && <Button variant="ghost" size="icon" className="absolute bottom-[50%] right-[calc(50%-1px)] z-10 hidden h-12 w-6 translate-y-1/2 rounded-none rounded-l-full border-l border-y border-white/10 bg-gray-800/50 hover:bg-gray-700/50 md:flex" onClick={togglePanel}>
