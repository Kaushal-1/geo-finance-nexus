    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#0a0e17] to-[#131b2e] overflow-x-hidden">
      ...
      <nav 
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${
          hasScrolled 
            ? "py-3 bg-black/70 backdrop-blur-md" 
            : "py-5 bg-transparent"
        }`}
      >
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="flex items-center space-x-2 pt-6">
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4].map((idx) => (
                    <div key={idx} className="w-8 h-8 rounded-full bg-gray-500 border-2 border-white/20"></div>
                  ))}
                </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, index) => (
              <div 
                key={index} 
                className="transition-all duration-500 opacity-0 translate-y-10" 
                style={{ 
